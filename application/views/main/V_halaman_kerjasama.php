...
<section id="hero">
    <div class="hero-container" data-aos="fade-in">
        <h1>Daftar Kerjasama Terkini</h1><br>
        <a href="#get-started" class="btn-get-started scrollto">Lihat Lebih Lanjut</a>
        <br>
        <br>
        <!--footer-->
        <footer>
    ...
    <section id="get-started" class="padd-section">
        <section id="about-us" class="about-us padd-section">
            <div class="container" data-aos="fade-up">
                <div class="row">
                    <?php $tahun = $this->input->get('year'); ?>
                    <h2>Tabel Kerjasama <?php echo $tahun ?></h2>
                    <p class="separator">Berikut adalah informasi kerjasama yang telah disetujui</p>


                    <form action="<?php echo base_url('kerjasama#get-started') ?>" style="margin-bottom: 60px;">
                        <div class="row">
                            <div class="col-md-2">
                                <div class="form-group">
                                    <select name="year" id="" class="form-control">
                                        ...
                                        for ($thn = $start; $thn <= intval($tahunSekarang); $thn++) {
                                            ...
                                            $selected = "";
                                            if (intval((20) . $thn) == intval($tahun)) {

                                                $selected = 'selected="selected"';
                                            }

                                            echo '<option value="20' . $thn . '" ' . $selected . '>20' . $thn . '</option>';
                                        }
                                        ?>
                                    </select>
                                </div>
                            ...
                            <div class="col-md-2">
                                <button class="btn btn-sm btn-secondary" style="border-radius: 5px !important; height: 40px; padding: 15px;">Lihat</button>
                            </div>
                        </div>
                        <small>Pilih tahun untuk menampilkan mitra kerjasama berdasarkan tahun pembuatan</small>
                    </form>
                        ...
                        <div class="col-md-12">
                            <table class="table table-bordered" id="table-mitra">
                                ...
                                <tbody>
                                    ...
                                    foreach ($dokumen->result_array() as $filter) {

                                        $tanggal_pembuatan = date('Y', strtotime($filter['created_at']));

                                        if ($tanggal_pembuatan == $tahun && $filter['status_persetujuan'] == "accept") {

                                            array_push($newDocument, $filter);
                                        }
                                    }


                                    if (count($newDocument) > 0) {
                                        foreach ($newDocument as $row) :
                                    ...
                                    ?>
                                            <tr>
                                                <td><?php echo $row['no_naskah'] ?></td>
                                                <td><?php echo $row['mitra'] ?></td>
                                                <td><?php echo $row['judul'] ?></td>
                                                <td><?php echo $tanggal ?></td>
                                                <td><label class="badge" style="background-color: <?php echo $color ?>;">
                                                        <?php echo $status ?></label>
                                                </td>
                                                <td>
                                                    <a download href="<?php echo base_url('assets/files/' . $row['file_acc']) ?>" class="btn-get-started scrollto">
                                                        <i style="font-size: 18px" class="bi bi-arrow-down-circle"></i>
                                                        Unduh File
                                                    </a>
                                                </td>
                                            </tr>
                                    <?php endforeach;
                                        ...
                                        echo '<tr>
                                            <td colspan="6" style="text-align: center">' . svg() . '<h4>Pencarian pada tahun ' . $tahun . '</h4><small>Permintaan anda tidak ditemukan</small></td>
                                        </tr>';
                                    }
                                    ?>
                                </tbody>
